...
;
util.inherits(GoogleAdapter, Adapter_1.default);
...
const getSingleMessageDetails = function (messageId, userEmail, token, apiVersion, additionalFields, result) {
    ...
    return request_promise_1.default(messageRequestOptions)
        .then((messageDetails) => {
...
const getAccessToken = function (clientId, adminEmail, userEmail, privateKey) {
    ...
    return request_promise_1.default(tokenRequestOptions)
        .then((body) => {
...
const getMoreEmails = function (messages, userEmail, token, apiVersion, additionalFields, emailRequestOptions, firstUri, nextPageToken) {
    emailRequestOptions.uri = firstUri + '&pageToken=' + nextPageToken;
    let tempPageToken = '';
    return request_promise_1.default(emailRequestOptions)
        .then((body) => {
...
const getUserEmails = function (clientId, serviceEmail, userEmail, privateKey, apiVersion, filterStartDate, filterEndDate, additionalFields, result) {
    let token = '';
    ...
    for (let userIter = 0; userIter < emailData.length; userIter++) {
        ...
        if (emailData[userIter].success) {
            for (let i = 0; i < emailData[userIter].data.messages.length; i++) {
                ...
                mappedEmailMessage.dateTimeSent = moment_1.default(new Date(getHeaderValue(messageData, 'Date'))).utc().toDate();
                const dateReceived = getHeaderValue(messageData, 'Received');
                if (dateReceived) {
                    const datePartOfValue = dateReceived.split(';')[1];
                    mappedEmailMessage.dateTimeReceived = moment_1.default(new Date(datePartOfValue)).utc().toDate();
                }
...
const getEmailData = function (emails, filterStartDate, filterEndDate, additionalFields, clientId, serviceEmail, privateKey, apiVersion) {
    const emailResults = [];
...
GoogleAdapter.prototype.runConnectionTest = function (connectionData) {
    ...
    const filterStartDate = moment_1.default().utc().startOf('day').add(-1, 'days').toDate();
    const filterEndDate = moment_1.default().utc().startOf('day').toDate();
...
GoogleAdapter.prototype.runMessageTest = function (connectionData) {
    ...
    _this._config = new GoogleMail.Configuration(connectionData.credentials);
    const filterStartDate = moment_1.default().utc().startOf('day').add(-1, 'days').toDate();
    const filterEndDate = moment_1.default().utc().startOf('day').add(1, 'days').toDate();
    return _this
